...
dub_int errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern dub_int syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern dub_int line; /* source code line number - defined in scanner.c */
...
extern dub_void startParser(boa_void);
extern dub_int startScanner(ReaderPointer sc_buf);

static dub_void printParserError(dub_char* fmt, ...);
static dub_void displayParser(ReaderPointer ptrBuffer);
static dub_long getParserFileSize(dub_char* fname);
static dub_void callGarbageCollector(boa_void);
...
dub_int mainParser(dub_int argc, dub_char** argv) {

	FILE* fi;       /* input file handle */
	dub_int loadsize = 0; /*the size of the file loaded in the buffer */
...
dub_void printParserError(dub_char* fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(dub_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
dub_long getParserFileSize(dub_char* fname) {
	FILE* input;
	dub_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
dub_void displayParser(ReaderPointer ptrBuffer) {
...
dub_void callGarbageCollector(boa_void) {
