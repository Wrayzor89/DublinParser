...
dub_int line;								/* Current line number of the source code */
extern dub_int errorNumber;				/* Defined in platy_st.c - run-time error number */

extern dub_int stateType[];
extern dub_char* keywordTable[];
extern PTR_ACCFUN finalStateTable[];
extern dub_int transitionTable[][TABLE_COLUMNS];
...
Token tokenizer(void) {
	...
	dub_char c;	/* input symbol */
	dub_int state = 0;		/* initial state of the FSM */
	dub_int lexStart;		/* start offset of a lexeme in the input char buffer (array) */
	dub_int lexEnd;		/* end offset of a lexeme in the input char buffer (array)*/

	dub_int lexLength;		/* token length */
	dub_int i;				/* counter */
	dub_char newc;			/* new char */
	...
	while (1) { /* endless loop broken by token returns it will generate a warning */
		...
		*/
		...
		default: // general case
			...
			lexemeBuffer = readerCreate((dub_int)lexLength + 2, 0, MODE_FIXED);
			if (!lexemeBuffer) {
...
dub_int nextState(dub_int state, dub_char c) {
	dub_int col;
	dub_int next;
	col = nextClass(c);
...
/*   [A-z] , [0-9],    $,    ",    _, \n,     #,       ,  other
      L(0),  D(1), C(2), Q(3), U(4), N(5),  H(6), S(7),  O(9)   */

dub_int nextClass(dub_char c) {
	dub_int val = -1;
	switch (c) {
	case CHRCOL2:
		val = 2;
...
Token funcIL(dub_char lexeme[]) {
	...
	if (lexeme[0] != '\0' && strlen(lexeme) > NUM_LEN) {
		...
		if (tlong >= 0 && tlong <= SHRT_MAX) {
			...
			currentToken.attribute.intValue = (dub_int)tlong;
		}
		else {
			currentToken = (*finalStateTable[ESNR])(lexeme);
		}
...
Token funcMID(dub_char lexeme[]) {
	...
	dub_char firstch = lexeme[0];
	dub_char nextch = lexeme[1];
	dub_int isID = DUB_FALSE;
	switch (firstch) {
	case MNIDPREFIX:
		if (nextch == MNIDPREFIX) {
...
Token funcSL(dub_char lexeme[]) {
	Token currentToken = { 0 };
	dub_int i = 0, len = (dub_int)strlen(lexeme);
	currentToken.attribute.contentString = readerGetPosWrte(stringLiteralTable);
	if (lexeme == -1) {
		printf("Scanner error, could not read lexeme \n");
	}
	if (lexeme == NULL) {
...
dub_void printToken(Token t) {
	extern dub_char* keywordTable[]; /* link to keyword table in */
	switch (t.code) {
	case RTE_T:
	...
	case STR_T:
		//printf("STR_T\t\t%d\t ", (dub_int)t.attribute.codeType);
		printf("STR_T\t\t%s\n", readerGetContent(stringLiteralTable, (dub_int)t.attribute.codeType));
		break;
	case LPR_T:
		printf("LPR_T\n");
		break;
